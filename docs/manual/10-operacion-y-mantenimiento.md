...
- **Registro de actividad**: con `APP_LOG=true` (valor por defecto) queda traza de las acciones de los usuarios, incluida la suplantación de identidad por administradores. Para las acciones de negocio principales (altas, modificaciones y bajas de partes, sanciones, comunicaciones, observaciones, estudiantes, docentes, catálogos y estructura académica) la entrada queda registrada de forma semántica —qué se creó, modificó o eliminó, y el valor antes/después de cada campo cambiado— en vez de solo la ruta y el código de estado HTTP. Solo se registran los campos relevantes para la auditoría (nunca, por ejemplo, la contraseña de un docente), y los borrados en cascada no generan una entrada por cada elemento eliminado, solo la acción principal. Las entradas se purgan automáticamente pasados los días configurados en el ajuste **Retención de los registros** (90 por defecto, limpieza semanal los domingos a las 3:00; ver [Ajustes](07-ajustes.md#notificaciones-a-familias)).

...
scripts `start.sh`/`start.ps1`/`start.bat` del paquete distribuido, ver
...
[Primeros pasos](02-primeros-pasos.md)). La opción `--demo-data` crea además un centro educativo de
demostración (código `23999999`, nombre «IES Test», ciudad «Linares») con un curso académico activo
nombrado con el año en curso (por ejemplo, «2026-2027») y sus conductas contrarias a la convivencia,
medidas disciplinarias y métodos de comunicación por defecto (ver
[Secciones de la aplicación](05-secciones-de-la-aplicacion.md)); solo tiene sentido para probar la
aplicación, nunca en un despliegue para un centro real.
